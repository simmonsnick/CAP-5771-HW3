...
def question1():
    ...
    answers["(a) explain"] = "In Agglomerative clustering, outliers are less problematic and it does not merge with any other clusters."
    "K-means has trouble clustering data that contain outliers."
    ...
    # type: explanatory string (at least four words)
    answers["(b) explain"] = "K-means has a number of limitations in finding different types of clusters im multiple runs."
    "Agglomonerative clustering merges the closest pairs but does not randomize the data points in each cluster."
    ...
    # type: explanatory string (at least four words)
    answers["(e) explain"] = "In K-means, Square Sum Error has a measure of the cohesion in clusters."
...
def question2():
    ...
    # type: explanatory string (at least four words)
    answers["(b) explain"] = "The shaded regions are near to each other and not in a round shape."

    # type: bool (True/False)
    answers["(c)"] = True

    # type: explanatory string (at least four words)
    answers["(c) explain"] = "In Figure C, has a point that's far away from any current cluster."
...
def question3():
    ...
    # type: a string that evaluates to a float
    answers["(c) SSE"] = "10R^2"
    #calculate all the regions that consist the points
    return answers
...
def question6():
    ...
    #core is the interior of a density-based cluster

    # type: set
    answers["(a) boundary"] = set(['D', 'G'])
    #border is not a core point, falls with the neighbor of a core point

    # type: set
    answers["(a) noise"] = set(['A', 'H'])
    #noise any point that's neither a core or border

    # type: set
    answers["(b) cluster 1"] = set(['B', 'C', 'D', 'E', 'F', 'G'])
    #Since point A is the farthest I would choose the closest points in the first cluster.
    #This cluster has more data points.

    # type: set
    answers["(b) cluster 2"] = set(['I', 'J', 'L', 'M'])
    #Since point H is the farthest I would choose the closest points in the second cluster.
...
def question8():
    ...
    # type: explanatory string (at least four words)
    answers["(a) explain diag entries, Matrix 1"] = "One diagonal entry that's blue."
    ...
    # type: explanatory string (at least four words)
    answers["(a) explain diag entries, Matrix 2"] = "Two diagonal entries are more blue and light blue compared to other patterns."
    ...
    # type: explanatory string (at least four words)
    answers["(a) explain diag entries, Matrix 3"] = "The blue diagonal entry that is compare to the previous two."

    # type: explanatory string (at least four words)
    answers["(a) explain non-diag entries, Matrix 3"] = "One row has a different color is non-diagonal. "
    #cluster cohesion how closely related the objects in a cluster
    # type: string
    answers["(b) Row 1"] = "1-100 cluster"
    #set of points contains 100 points
    # type: string
...
def question9():
    ...
    # type: explanatory string (at least four words)
    answers["(e) explain"] = "Can't be grouped with students didn't get in by grade in data mining."
...
def question10():
    ...
    # type: string
    answers["(a) Figure (a)"] = "The noise is much dense than the specific patterns so eyes and mouth will be out."
    
    # type: string
    answers["(a) Figure (b)"] = "The points in nose, eyes, and mouth are nearby and DBSCAN works for the figure b."

    # type: explanatory string (at least four words)
    answers["(a) explain"] = "DBSCAN can be use to identify the clusters."

    # type: string
    answers["(b) Figure (a)"] = "In figure, K-means does not work for figure a."

    # type: string
    answers["(b) Figure (b)"] = "In figure, K-means does work for figure b."

    # type: explanatory string (at least four words)
    answers["(b) explain"] = "You can't use K-mean for every figure with the placement in certain data points to the cluster."

    # type: string
    answers["(c)"] = "The density-based measure being use in DBSCAN can find the patterns in nose, eyes, and mouth."
